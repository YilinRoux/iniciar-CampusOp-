...
async function call(path, status, actor, body, key) {
  ...
  assert.equal(
    response.status,
    status,
    `${path}: status esperado ${status}, recibido ${response.status}`,
  );
...
try {
  await call(
    "/v1/incidents/campus-inc-001/actions",
    403,
    "technician-1",
    { action: "assign", baseVersion: 1, technicianId: "technician-2" },
    "unauthorized-assign-attempt",
  );
  console.log(
    "OK: un tecnico sin rol de coordinador no puede ejecutar la accion assign (403 confirmado).",
  );
} finally {
  child.kill();
}
