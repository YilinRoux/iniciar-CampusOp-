...
const REFRESH_NEXT_TOKEN = process.env.COURSE_REFRESH_NEXT_TOKEN;
...
const server = createServer(async (request, response) => {
  ...
  const scenario =
    request.headers['x-course-scenario'] ?? 'success';
  ...
  if (
    request.method === 'POST' &&
    url.pathname === '/v1/session/refresh'
  ) {
    ...
    return send(response, 200, {
      ...
      refreshToken: REFRESH_NEXT_TOKEN,
      expiresIn: 60,
    });
  }
  ...
  if (
    request.method === 'POST' &&
    url.pathname === '/v1/resources/action'
  ) {
