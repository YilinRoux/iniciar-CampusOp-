...
function visible(actorId, incident) {
  return (
    actors[actorId] === 'coordinator' ||
    (actors[actorId] === 'reporter' &&
      incident.payload.reporterId === actorId) ||
    (actors[actorId] === 'technician' &&
      incident.payload.assignedTechnicianId === actorId)
  );
}

export async function handleCampusOps(
  request,
  response,
  url,
  { send, readJson, scenario }
) {
  ...
  if (request.method === 'GET' && url.pathname === '/v1/geocoding') {
    if (!nonempty(url.searchParams.get('q'))) {
      return send(response, 422, {
        code: 'query_required',
      });
    }

    if (scenario === 'incomplete' || scenario === 'nullable') {
      return send(response, 200, {
        label: 'Zona ficticia',
      });
    }
  ...
  if (request.method === 'GET') {
    ...
    if (incident && !visible(actorId, incident)) {
    ...
    const project = (item) => ({
      ...item,
      payload: scenario === 'nullable' ? null : item.payload,
    });
      ...
      incident
        ...
        : {
            items: [...incidents.values()]
              .filter((item) => visible(actorId, item))
              .map(project),
          }
    );
  }
  ...
  if (!match[1]) {
    ...
    result = {
      incident: structuredClone(created),
      operationId: key,
      duplicate: false,
    };
  } else {
    ...
    if (
      (coordinatorActions.includes(action) &&
        role !== 'coordinator') ||
      (technicianActions.includes(action) &&
        (role !== 'technician' ||
          !visible(actorId, incident))) ||
      (sharedActions.includes(action) &&
        !visible(actorId, incident))
    ) {
      return send(response, 403, {
        code: 'forbidden',
    ...
    if (input.baseVersion !== incident.version) {
      return send(response, 409, {
    ...
    switch (action) {
      case 'assign':
        if (actors[input.technicianId] !== 'technician') {
          return send(response, 422, {
      ...
      case 'reopen':
        if (!['closed', 'resolved'].includes(next.status)) {
          return send(response, 409, {
            code: 'transition_conflict',
          });
        }

        next.status = next.payload.assignedTechnicianId
          ? 'assigned'
          : 'open';
        break;

      case 'comment':
        if (!nonempty(input.text)) {
          return send(response, 422, {
            code: 'text_required',
          });
        }

        next.payload.notes.push({
          actorId,
          text: input.text,
        });
        break;
